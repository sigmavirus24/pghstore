...
try:
    from pghstore import _rust_ffi as pghstorers
except ImportError:
    pghstorers = None
...
tmpl = ".. n=%i, strlen=%i, %s.loads (%.2fs) is %ix faster than %s.loads (%.2fs)"
tmpl_2 = ".. n=%i, dictlen=%i, pghstore.loads (%.2fs)"
    ...
    def test_loads(self):
        ...
        for name in names:
            ...
            if cpghstore is not None:
                ...
                print(
                    tmpl % (
                        n, len(name), 'cpghstore', cpg_time,
                        floor(pg_time / cpg_time), 'pghstore', pg_time,
                    )
                )
            else:
                print(tmpl_2 % (n, len(name), pg_time))

            if pghstorers is not None:
                rspg_time = timeit.timeit(partial(pghstorers.loads, name), number=n)
                #self.assertTrue(rspg_time < pg_time)

                print(
                    tmpl % (
                        n, len(name), 'pghstorers', rspg_time,
                        floor(pg_time / rspg_time), 'pghstore', pg_time,
                    )
                )
                if cpghstore is not None:
                    print(
                        tmpl % (
                            n, len(name), 'pghstorers', rspg_time,
                            floor(cpg_time / rspg_time), 'cpghstore', cpg_time,
                        )
                    )
            else:
                print(tmpl_2 % (n, len(name), pg_time))
